...
//! workspace binary.
//!
//! Set `GIT_MEMORY_RELEASE_BINARY` to the path of a release binary. When
...
fn release_artifact_passes_contract() {
    let binary = match env::var("GIT_MEMORY_RELEASE_BINARY") {
        Ok(path) if !path.is_empty() => path,
        _ => {
            eprintln!("skipping: GIT_MEMORY_RELEASE_BINARY not set");
            return;
        }
    };
