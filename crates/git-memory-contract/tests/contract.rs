...
use git_memory_contract::{FakeServerTarget, ReleaseBinaryTarget, run_contract};
use serde_json::Value;

fn fake_binary() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_git-memory-contract-fake"))
}
...
fn command_line_runner_emits_a_machine_readable_report() {
    let output = Command::new(env!("CARGO_BIN_EXE_git-memory-contract"))
        .arg("--fake-binary")
        .arg(fake_binary())
        .args(["--output", "json"])
        .output()
