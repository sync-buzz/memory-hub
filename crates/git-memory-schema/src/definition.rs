...
impl TypeDefinition {
    ...
    ///
    /// Checks `kind_name` is non-empty, every field type is internally
    /// consistent (enum has values, array has items), envelope constraints only
    /// reference known fields, and relationship targets are non-empty.
    ///
