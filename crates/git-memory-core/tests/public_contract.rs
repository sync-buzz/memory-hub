...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(wire["source_paths"]["scope"][0], "crates/git-memory-core/");
...
fn encrypted_wire_rejects_an_incompatible_major_before_storage() {
    ...
    let Err(error) = serde_json::from_value::<git_memory_core::StoredRecord>(value) else {
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let resolver = PolicyResolver::git_memory_defaults().with_project(project)?;
