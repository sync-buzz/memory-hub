//! Schema validation integration tests for the Git store.
//!
...
use git_memory_store::{GitStore, Operation, RecordId, StoreError, StoreErrorKind, Transaction};
...
fn revision(store: &GitStore) -> git_memory_store::Revision {
...
fn type_record(kind_name: &str, content: &str) -> StoredRecord {
    let key = git_memory_schema::type_key(kind_name);
...
fn apply(
    ...
    id: &str,
    ops: Vec<Operation>,
) -> Result<git_memory_store::ApplyResult, StoreError> {
...
fn create_type_record_succeeds() {
    ...
    let registry = store.load_schema_registry(&revision(&store)).unwrap();
...
fn delete_type_record_then_kind_rejected_in_strict_mode() {
    ...
    let type_key = git_memory_schema::type_key("decision");
    apply(
        &store,
        "tx-delete-type",
...
fn reject_undeclared_link_relation() {
    ...
    env.links.push(git_memory_core::RecordLink {
        key: "specs/foo".into(),
