...
//! one thread — needs to own a `Session` inside a task. A missing bound
//! anywhere in that chain makes the whole thing unusable, and nothing in the
//! type system says so until a caller tries. This test says it at compile
//! time.
