...
/// The dates are the history's, read back from the commits themselves rather
/// than compared with the clock — two transactions a test apart land in the
/// same second, and an assertion that one is later than the other would pass
/// on a slow machine and fail on a fast one.
#[test]
fn record_times_are_the_transactions_that_named_the_key() -> Result<(), Box<dyn std::error::Error>>
{
    let (directory, store) = repository()?;
    let first = store.apply(&transaction(
        &store,
        ...
        vec![
            ...
            Operation::put(record("beta", "only ever written once")?),
        ],
    )?)?;
    let second = store.apply(&transaction(
        &store,
        "second",
        vec![Operation::put(record("alpha", "two")?)],
    )?)?;

    let repository = Repository::open(directory.path())?;
    let time_of = |revision: &Revision| -> Result<i64, Box<dyn std::error::Error>> {
        Ok(repository
            .find_commit(git2::Oid::from_str(revision.as_str())?)?
            .time()
            .seconds())
    };
    let times = store.record_times(&second.revision)?;

    let alpha = times
        .get(&RecordId::plaintext("alpha"))
        .ok_or("alpha has no times")?;
    assert_eq!(alpha.created_at_epoch_seconds, time_of(&first.revision)?);
    assert_eq!(alpha.updated_at_epoch_seconds, time_of(&second.revision)?);

    // A record the second transaction did not touch keeps the first one for
    // both: the walk records what named the key, and nothing else moves it.
    let beta = times
        .get(&RecordId::plaintext("beta"))
        .ok_or("beta has no times")?;
    assert_eq!(beta.created_at_epoch_seconds, time_of(&first.revision)?);
    assert_eq!(beta.updated_at_epoch_seconds, time_of(&first.revision)?);
    Ok(())
}

/// The dates are derived and are not part of what a record is. A write that
/// carried them would put the chain's own account of itself inside the record
/// it describes — and, worse, would give a record re-stated verbatim different
/// bytes, so a write that changed nothing would land in the history as a
/// change.
#[test]
fn the_dates_never_reach_the_blob() -> Result<(), Box<dyn std::error::Error>> {
    let (directory, store) = repository()?;
    let mut envelope = Envelope::new("alpha", "note", "first")?;
    envelope.created_at_epoch_seconds = Some(1);
    envelope.updated_at_epoch_seconds = Some(2);
    ...
    let repository = Repository::open(directory.path())?;
    let tree = repository
        .find_commit(git2::Oid::from_str(written.revision.as_str())?)?
        .tree()?;
    let mut bodies = Vec::new();
    tree.walk(git2::TreeWalkMode::PreOrder, |_, entry| {
        if let Some(object) = entry.to_object(&repository).ok()
            && let Some(blob) = object.as_blob()
        {
            bodies.push(String::from_utf8_lossy(blob.content()).into_owned());
        }
        git2::TreeWalkResult::Ok
    })?;
    assert_eq!(bodies.len(), 1, "one record was written");
    assert!(
        !bodies[0].contains("created_at_epoch_seconds")
            && !bodies[0].contains("updated_at_epoch_seconds"),
        "the stored record states a date: {}",
        bodies[0]
    );
    Ok(())
}
