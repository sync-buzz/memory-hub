...
fn seed_store(
    dir: &std::path::Path,
) -> Result<(GitStore, git_memory_store::Revision), Box<dyn std::error::Error>> {
    Repository::init(dir)?;
...
fn search_request(query: &str, revision: &git_memory_store::Revision) -> SearchRequest {
    SearchRequest {
...
async fn rrf_fuse_combines_bm25_and_vector_channels() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let alpha = result.hits.iter().find(|h| h.id == "alpha").unwrap();
