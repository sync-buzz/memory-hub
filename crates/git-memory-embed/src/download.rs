...
//!   2. `$GIT_MEMORY_MODEL_CACHE`,
//!   3. [`dirs::cache_dir`] joined with `git-memory/models`.
//!
...
const ENV_CACHE_DIR: &str = "GIT_MEMORY_MODEL_CACHE";
...
pub fn cache_dir_root(opts: &DownloadOpts) -> Result<PathBuf, DownloadError> {
    ...
    dirs::cache_dir()
        .map(|d| d.join("git-memory").join("models"))
...
fn default_client() -> reqwest::Client {
    reqwest::Client::builder()
        .user_agent(concat!("git-memory/", env!("CARGO_PKG_VERSION")))
        .build()
