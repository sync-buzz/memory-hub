...
fn field_type_label(field_type: &git_memory_schema::FieldType) -> String {
    match field_type {
        git_memory_schema::FieldType::String => "string".into(),
        git_memory_schema::FieldType::Text => "text".into(),
        git_memory_schema::FieldType::Integer => "integer".into(),
        git_memory_schema::FieldType::Number => "number".into(),
        git_memory_schema::FieldType::Boolean => "boolean".into(),
        git_memory_schema::FieldType::Enum { values } => {
            format!("enum: {}", values.join(" | "))
        }
        git_memory_schema::FieldType::Array { items } => {
            format!("array<{}>", field_type_label(items))
        }
        git_memory_schema::FieldType::Object => "object".into(),
    }
}
