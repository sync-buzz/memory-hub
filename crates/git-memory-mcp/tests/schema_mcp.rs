...
use git_memory_schema::{SchemaRegistry, TypeDefinition};
use git_memory_store::{GitStore, Operation};
...
fn schema_instructions_empty_for_empty_registry() {
    ...
    let text = git_memory_mcp::schema_instructions(&registry);
...
fn schema_resource_returns_all_types() {
    let registry = registry_with_types();
    let resource = git_memory_mcp::schema_resource(&registry);
...
fn single_type_resource_returns_one_definition() {
    ...
    let resource = git_memory_mcp::single_type_resource(definition);
    assert_eq!(resource["schemaVersion"], 1);
...
fn setup_project() -> (tempfile::TempDir, git_memory_mcp::Session) {
    ...
    let tx = git_memory_store::Transaction {
        id: "setup-types".into(),
    ...
    let session = git_memory_mcp::Session::new(project.path().to_path_buf());
    (project, session)
}
...
fn initialize_includes_schema_instructions_when_types_exist() {
    ...
    let init_result = session.initialize(&json!({
        "protocolVersion": git_memory_mcp::MCP_PROTOCOL_VERSION,
        "capabilities": {},
        "clientInfo": {"name": "test", "version": "1"},
        "_meta": {"gitMemory": {"memoryInterfaceVersion": {"major": git_memory_mcp::MEMORY_INTERFACE_MAJOR, "minor": 0}}}
    }));
...
fn initialize_without_types_has_only_builtin_instructions() {
    let project = tempfile::tempdir().unwrap();
    init_git(project.path());
    let mut session = git_memory_mcp::Session::new(project.path().to_path_buf());
    ...
    let instructions = result["instructions"].as_str().unwrap();
    assert!(!instructions.contains("## Document Types"));
    assert!(instructions.contains("Git Memory"));
}
...
fn memory_schema_status_reports_no_incompatibles_when_valid() {
    ...
    store
        .apply(&git_memory_store::Transaction {
            id: "tx-valid".into(),
            expected_revision: revision,
...
fn memory_schema_status_reports_incompatible_records() {
    ...
    // Write a record with unknown kind using non-strict store (bypasses validation).
    let store = GitStore::open(session.project.as_path())
        .unwrap()
        .with_schema_strict(false);
    let snapshot = store.current().unwrap();
    let revision = snapshot.revision().clone();
    let record = StoredRecord::Plaintext {
...
fn list_tools_includes_schema_tools() {
    let tools = git_memory_mcp::list_tools();
...
fn list_resources_includes_schema_resource() {
    let resources = git_memory_mcp::list_resources();
...
fn list_resource_templates_includes_schema_template() {
    let templates = git_memory_mcp::list_resource_templates();
