...
//!
//! Drives the real `git memory mcp` stdio server over JSON-RPC so the
...
impl McpSession {
    ...
    fn call_tool(&mut self, name: &str, arguments: Value) -> Value {
        ...
        let content = result.get("structuredContent").cloned().unwrap_or(result);
        content
    }
...
fn full_record_includes_content_and_links_when_not_metadata_only() {
    ...
    assert!(
        record.get("freshness").map_or(false, |v| v.is_object()),
