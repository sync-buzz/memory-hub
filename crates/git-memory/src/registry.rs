//! Installation registry for shared git-memory lifecycle.
//!
//! Tracks which consumers (Sync, other clients) depend on the git-memory
//! installation and which repositories use it. Lives at
//! `config_dir()/git-memory/registry.json`, overridable via
//! `$GIT_MEMORY_CONFIG_DIR`.
//!
...
/// 1. `$GIT_MEMORY_CONFIG_DIR` (explicit override)
/// 2. `dirs::config_dir()/git-memory`
...
/// A registered consumer of the git-memory installation.
...
/// Detect the binary path of the currently running git-memory.
#[must_use]
...
mod tests {
    ...
    fn is_compatible_with_same_major() {
        ...
        registry.set_installation(InstallationRecord {
            version: "0.1.0".into(),
            binary_path: "/usr/local/bin/git-memory".into(),
