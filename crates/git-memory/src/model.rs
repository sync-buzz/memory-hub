//! `git memory model` subcommand handlers.
//!
//! Five commands:
//!
    ...
    match output {
        Output::Human => {
            ...
            if !missing.is_empty() {
                ...
                for row in &missing {
                    println!(
                        "  Hint: download with `git memory model download {}`",
                        row.id
                    );
                }
            }
...
pub(crate) fn use_model(id: &str, output: Output) -> Code {
    ...
    if let Err(error) = config::set_active_model(id) {
        ...
        match output {
            Output::Human => eprintln!("git-memory: {msg}"),
            Output::Json => {
                let _ = print_error_json(&msg);
            }
        }
    ...
    match output {
        Output::Human => {
            ...
            if !on_disk {
                println!(
                    "  Hint: model is not on disk — download with `git memory model download {}`",
                    entry.id
                );
            }
            Code::Success
        ...
        Output::Json => {
            println!(
                ...
                serde_json::json!({
                    ...
                    "on_disk": on_disk,
                    "hint": if on_disk { None } else { Some(format!("git memory model download {}", entry.id)) },
                })
            );
            Code::Success
        }
...
pub(crate) fn benchmark(id: &str, output: Output) -> Code {
    ...
    match verify_model_sync(entry, &opts) {
        ...
        Ok(ModelVerification::Missing) => {
            let msg = format!(
                "model '{}' is not on disk — download with `git memory model download {}`",
                entry.id, entry.id
            );
            match output {
                Output::Human => eprintln!("git-memory: {msg}"),
        ...
        Ok(ModelVerification::Broken { .. }) => {
            let msg = format!(
                "model '{}' is on disk but its hash does not match — re-download with `git memory model download {}`",
                entry.id, entry.id
            );
            match output {
                Output::Human => eprintln!("git-memory: {msg}"),
...
pub(crate) fn doctor_check() -> DoctorModelCheck {
    ...
    match verify_model_sync(active, &opts) {
        ...
        Ok(ModelVerification::Missing) => DoctorModelCheck {
            ...
            message: format!(
                "model '{}' is not on disk — run `git memory model download {}`",
                active.id, active.id
            ),
        },
        Ok(ModelVerification::Broken { .. }) => DoctorModelCheck {
            ...
            message: format!(
                "model '{}' is on disk but hash does not match — re-download with `git memory model download {}`",
                active.id, active.id
            ),
        },
