...
/// Handle a `git memory registry` subcommand.
...
pub fn handle_uninstall(purge: bool, yes: bool, output: Output) -> Code {
    ...
    if let Err(e) = registry.save() {
        eprintln!("git-memory: cannot save registry: {e}");
        return Code::Internal;
    }
    ...
    match output {
        ...
        Output::Human => {
            println!("git-memory uninstalled from registry.");
            if purge {
                println!("All git-memory data has been purged.");
            } else {
...
fn unregister_consumer(name: &str, output: Output) -> Code {
    ...
    match output {
        ...
        Output::Human => {
            ...
            println!("git-memory binary and data are NOT removed.");
        }
    }
    Code::Success
}
...
/// Self-register the current binary as the installation. Called when git-memory
