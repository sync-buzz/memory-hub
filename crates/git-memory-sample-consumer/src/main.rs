//! Independent sample consumer for Git Memory.
//!
...
//! 1. Launch `git-memory mcp` as a subprocess
...
//!
//! Usage:
//!   git-memory-sample-consumer <path-to-git-memory-binary> <project-path>
...
impl McpClient {
    /// Launch `git-memory mcp --project <project>` as a subprocess.
    fn launch(binary: &str, project: &str) -> Result<Self, String> {
        let mut child = Command::new(binary)
            ...
            .spawn()
            .map_err(|e| format!("failed to launch git-memory: {e}"))?;
...
fn run_sample(binary: &str, project: &str) -> Result<(), String> {
    println!("Launching git-memory MCP server...");
    ...
    let hit_count = search_data["hits"].as_array().map(Vec::len).unwrap_or(0);
    ...
    let record_count = export_data["records"].as_array().map(Vec::len).unwrap_or(0);
...
/// Simple SHA-256 hex (avoids pulling in sha2 crate for this minimal consumer).
fn sha256_hex(input: &str) -> String {
    // This is a simplified hash for demonstration — in production, use the
    // sha2 crate. For the sample consumer, we use a basic FNV-like hash
    // that is NOT cryptographically secure but suffices for the demo.
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("Usage: {} <git-memory-binary> <project-path>", args[0]);
        std::process::exit(1);
    }
