...
use fs2::FileExt;
use git_memory_core::{FreshnessState, StoredRecord};
use git_memory_store::{Checkpoint, GitStore, Operation, StoreError, Transaction};
...
impl Reconciler {
    ...
    pub fn reconcile(&self, mode: DivergenceMode) -> Result<ReconcileReport, ReconcileError> {
        ...
        for oid in commits {
            ...
            let stale_keys = stale_records(&store, oid, &changed_paths)?;
...
fn paths_overlap(envelope: &git_memory_core::Envelope, changed: &[String]) -> bool {
...
fn cursor_path(repository: &Repository) -> PathBuf {
    repository.path().join("git-memory/reconcile-cursor.json")
}
