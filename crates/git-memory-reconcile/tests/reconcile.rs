...
use git_memory_core::{Envelope, FreshnessState, StoredRecord};
use git_memory_reconcile::{DivergenceMode, ReconcileErrorKind, Reconciler};
use git_memory_store::{GitStore, Operation, RecordId, Transaction};
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let primary_cursor = repository.path().join("git-memory/reconcile-cursor.json");
    let linked_cursor = linked.path().join("git-memory/reconcile-cursor.json");
